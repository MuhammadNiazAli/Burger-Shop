  ...
  <header class="sticky top-0 z-40 border-b border-charcoal/10 bg-paper/95 backdrop-blur">
    <div class="mx-auto max-w-6xl px-5 sm:px-8 flex items-center justify-between h-20">
      ...
      <div v-if="currentUser" class="flex items-center gap-4">
        ...
        <NuxtLink to="/profile" class="flex items-center gap-2.5 group" aria-label="Your profile">
          <img
          ...
          />
          <span class="hidden sm:inline text-sm font-semibold group-hover:text-flame transition-colors">{{ currentUser.name.split(' ')[0] }}</span>
        </NuxtLink>

        <button class="hidden sm:block text-[11px] font-mono uppercase text-smoke hover:text-flame transition-colors" @click="doLogout">Sign out</button>
